...
const AdminDashboard = () => {
  ...
  if (isLoading || isOrdersLoading || isUsersLoading)
    return <div>Loading...</div>;

  const delivered =
    orders.data.length > 0 &&
    orders.data.filter((order: TOrder) => order.isDelivered);
  const paid =
    orders.data.length > 0 &&
    orders.data.filter((order: TOrder) => order.isPaid);
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        ...
        <Card className="col-span-3">
          <CardHeader>
            ...
            <CardDescription>
              You made {paid.length} sales this month.
            </CardDescription>
          </CardHeader>
          <CardContent>
